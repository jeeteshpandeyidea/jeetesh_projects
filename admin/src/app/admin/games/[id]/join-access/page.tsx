...
export default function GameJoinAccessPage() {
  ...
  const handleGameUpdated = useCallback(() => {
    fetchGame().then((data) => data && setGame(data));
  }, [fetchGame]);

  if (loading) {
    return (
      <div>
        <PageBreadcrumb pageTitle="Join & Access" />
    ...
    <div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-semibold text-gray-800 dark:text-white/90">Join & Access</h1>
        ...
        <ComponentCard title={`Game: ${game.name ?? '—'} (${game.game_code ?? '—'})`}>
          <GameJoinAccessCard
            game={game}
