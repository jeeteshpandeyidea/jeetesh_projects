...
export default function AddGameForm() {
  ...
  const categoryOptions = categories.map((c) => ({ value: (c as { _id?: string })._id ?? (c as { id: string }).id, label: c.name }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        game_start_date: formData.game_start_date ? `${formData.game_start_date}T00:00:00.000Z` : undefined,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Add Game</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <div className="relative">
            <Select options={[{ value: '', label: 'Select Category' }, ...categoryOptions]} placeholder="Select Category" onChange={handleSelect('category_id')} defaultValue={formData.category_id} className="dark:bg-dark-900" />
            <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2"><ChevronDownIcon /></span>
          </div>
        </div>
        <div>
          <Label>Grid Size <span className="text-red-500">*</span></Label>
          <div className="relative">
            <Select options={[{ value: '', label: 'Select Grid Size' }, ...gridSizeOptions]} placeholder="Select Grid Size" onChange={handleSelect('grid_size_id')} defaultValue={formData.grid_size_id} className="dark:bg-dark-900" />
            <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2"><ChevronDownIcon /></span>
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            <Select options={[{ value: '', label: 'Select Card Gen Type' }, ...cardGenOptions]} placeholder="Select Card Gen Type" onChange={handleSelect('card_gen_id')} defaultValue={formData.card_gen_id} className="dark:bg-dark-900" />
            <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2"><ChevronDownIcon /></span>
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            <Select options={[{ value: '', label: 'Select Game Type' }, ...gameTypeOptions]} placeholder="Select Game Type" onChange={handleSelect('game_type_id')} defaultValue={formData.game_type_id} className="dark:bg-dark-900" />
            <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2"><ChevronDownIcon /></span>
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            <Select options={[{ value: '', label: 'Select Winning Pattern' }, ...winningPatternOptions]} placeholder="Select Winning Pattern" onChange={handleSelect('winning_patt_id')} defaultValue={formData.winning_patt_id} className="dark:bg-dark-900" />
            <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2"><ChevronDownIcon /></span>
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            <Select
              ...
              defaultValue={formData.winning_pattern_type_id}
        ...
        <div>
          ...
          <div className="relative">
            <Select options={accessOptions} placeholder="Open/Closed" onChange={(v) => setFormData((p) => ({ ...p, access_control: v === 'true' }))} defaultValue={String(formData.access_control)} className="dark:bg-dark-900" />
            <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2"><ChevronDownIcon /></span>
          </div>
        </div>
        <div>
          <Label htmlFor="max_player">Max Player <span className="text-red-500">*</span></Label>
          <Input id="max_player" name="max_player" type="number" min="1" value={formData.max_player} onChange={handleInputChange} placeholder="e.g. 10" disabled={isSubmitting} required />
        </div>
        <div>
        ...
        <div>
          ...
          <div className="relative">
            <Select options={statusOptions} placeholder="Status" onChange={handleSelect('status')} defaultValue={formData.status} className="dark:bg-dark-900" />
            <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2"><ChevronDownIcon /></span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-white/[0.05]">
        <Link href={ROUTES.GAMES}><Button type="button" size="sm" variant="outline" disabled={isSubmitting}>Cancel</Button></Link>
        <Button type="submit" size="sm" disabled={isSubmitting}>{isSubmitting ? 'Creating...' : 'Create Game'}</Button>
      </div>
    </form>
  );
}
