...
export default function AddRewardForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.REWARDS.CREATE, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        const err = await response.json().catch(() => ({ message: 'Failed to create reward' }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Add Reward</h3>
      ...
      <div className="space-y-4">
        <div>
          ...
          <Input id="name" name="name" type="text" value={formData.name} onChange={handleInputChange} placeholder="Enter reward name" required disabled={isSubmitting} />
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Slug will be auto-generated from name (unique).</p>
        </div>
        <div>
          <Label htmlFor="value">Value</Label>
          <Input id="value" name="value" type="number" value={formData.value} onChange={handleInputChange} placeholder="e.g. 100" disabled={isSubmitting} />
        </div>
        <div>
          <Label>Status</Label>
      ...
      <div className="flex items-center justify-end w-full gap-3 pt-4 border-t border-gray-200 dark:border-white/[0.05]">
        <Link href={ROUTES.REWARDS}><Button type="button" size="sm" variant="outline" disabled={isSubmitting}>Cancel</Button></Link>
        <Button type="submit" size="sm" disabled={isSubmitting}>{isSubmitting ? 'Creating...' : 'Create Reward'}</Button>
      </div>
    </form>
  );
}
