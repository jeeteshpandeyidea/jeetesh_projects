...
export default function EditRewardForm({ reward }: EditRewardFormProps) {
  ...
  const [formData, setFormData] = useState({ name: '', value: '', status: 'active' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.REWARDS.UPDATE(reward._id), {
        method: 'PATCH',
      ...
      if (!response.ok) {
        ...
        const err = await response.json().catch(() => ({ message: 'Failed to update reward' }));
        throw new Error(err.message || 'Failed to update reward');
      }
      showSuccessToast('Reward updated successfully', 'Success');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Edit Reward</h3>
