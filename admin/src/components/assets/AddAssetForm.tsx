...
export default function AddAssetForm({ onSuccess, onCancel }: AddAssetFormProps) {
  ...
  /** Extract a string ID from category/subcategory (handles _id as string or object e.g. MongoDB) */
  const getId = (o: Record<string, unknown>, key: string): string => {
    const v = o[key] ?? o[key === '_id' ? 'id' : '_id'];
    if (v == null) return '';
    if (typeof v === 'string') return v.trim();
    if (typeof v === 'object' && v !== null && '$oid' in v) return String((v as { $oid: string }).$oid).trim();
    return String(v).trim();
  };
  const getCategoryId = (c: Record<string, unknown>) => getId(c, '_id');
  const categoryOptions = categories
    .filter((c) => getCategoryId(c as Record<string, unknown>))
    .map((c) => ({
      value: getCategoryId(c as Record<string, unknown>),
  ...
  const getSubCategoryCategoryId = (s: { category_id?: string | { _id?: string } }) => {
    ...
    if (!cid) return '';
    return typeof cid === 'string' ? cid : getId(cid as Record<string, unknown>, '_id');
  };
  const filteredSubcategories = formData.categoryId
    ? subcategories.filter((s) => getSubCategoryCategoryId(s) === formData.categoryId)
    : [];
  const getSubcategoryId = (s: Record<string, unknown>) => getId(s, '_id');
  const subcategoryOptions = filteredSubcategories
    .filter((s) => getSubcategoryId(s as Record<string, unknown>))
    .map((s) => ({
      value: getSubcategoryId(s as Record<string, unknown>),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errData = await response.json().catch(() => ({ message: 'Failed to create asset' }));
        throw new Error(errData.message || 'Failed to create asset');
      }
      showSuccessToast('Asset created successfully', 'Success');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Add Asset</h3>
        {isPageMode && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2 p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-white/10">
          <p className="text-sm text-gray-600 dark:text-gray-400">A unique code (e.g. <strong>BNG001</strong>, BNG002, …) will be generated when you save.</p>
        </div>
        ...
        <div>
          ...
          <div className="relative" key={formData.categoryId || 'no-cat'}>
            <Select
              options={[{ value: '', label: formData.categoryId ? 'Select subcategory (optional)' : 'Select category first' }, ...subcategoryOptions]}
              placeholder="Subcategory"
              onChange={handleSelect('subcategoryId')}
        ...
        <div>
          ...
          <div className="relative">
            <Select options={accessLevelOptions} placeholder="Access Level" onChange={handleSelect('accessLevel')} defaultValue={formData.accessLevel} className="dark:bg-dark-900" />
            <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none right-3 top-1/2 dark:text-gray-400"><ChevronDownIcon /></span>
          </div>
        </div>
        <div>
      ...
      <div className="flex items-center justify-end w-full gap-3 pt-4 border-t border-gray-200 dark:border-white/[0.05]">
        ...
        <Button type="submit" size="sm" disabled={isSubmitting}>{isSubmitting ? 'Creating...' : 'Create Asset'}</Button>
      </div>
    </form>
  );
}
