...
export default function AddEventForm() {
  ...
  const selectedPatternName = formData.winning_pattern_id
    ...
    : '';
  const winningConditionPayload = selectedPatternName || '';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Add Event</h3>
      ...
      <div className="flex items-center justify-end w-full gap-3 pt-4 border-t border-gray-200 dark:border-white/[0.05]">
        ...
        <Button type="submit" size="sm" disabled={isSubmitting}>{isSubmitting ? 'Creating...' : 'Create Event'}</Button>
      </div>
    </form>
  );
}
