...
interface EditWinningPatternFormProps {
  winningPattern: WinningPattern;
}

export default function EditWinningPatternForm({ winningPattern }: EditWinningPatternFormProps) {
  const router = useRouter();
  const [formData, setFormData] = useState({ name: '', status: 'active' });

  useEffect(() => {
    setFormData({
      name: winningPattern.name || '',
      status: winningPattern.status || 'active',
    });
  }, [winningPattern]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.WINNING_PATTERN.UPDATE(winningPattern._id), {
        method: 'PATCH',
      ...
      if (!response.ok) {
        ...
        const errorData = await response.json().catch(() => ({ message: 'Failed to update winning pattern' }));
        throw new Error(errorData.message || 'Failed to update winning pattern');
      }
      showSuccessToast('Winning pattern updated successfully', 'Success');
      router.push(ROUTES.WINNING_PATTERN);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to update winning pattern');
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Edit Winning Pattern</h3>
